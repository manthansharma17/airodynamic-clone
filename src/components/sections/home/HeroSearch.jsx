...
export default function HeroSearch() {
  // "return" | "oneway" — controls the sliding pill + label colors below.
  const [tripType, setTripType] = useState("return");

  return (
    <>
      {/* Desktop only (1024px+). Tablet/mobile use the Book Flight drawer instead. */}
      <div className="hero-search absolute bottom-15 left-1/2 z-50 hidden w-full max-w-6xl -translate-x-1/2 px-6 lg:block">
        {/* Wrapper */}
        <div className="relative pt-10">
          ...
          <div className="absolute left-1/2 top-0 z-50 -translate-x-1/2 -translate-y-1/2">
            <div className="relative inline-flex rounded-full bg-white p-1 shadow-xl">
              {/* Active Background — slides between the two tabs */}
              <div
                className={`absolute inset-y-1 left-1 w-[calc(50%-4px)] rounded-full bg-[#0B2D63] transition-transform duration-300 ease-out ${
                  tripType === "oneway" ? "translate-x-full" : "translate-x-0"
                }`}
              />

              <button
                onClick={() => setTripType("return")}
                className={`relative z-10 h-10 w-32 rounded-full text-sm font-medium transition-colors duration-300 ${
                  tripType === "return"
                    ? "text-white"
